...
/**
 * Preserves publisher styling while removing EPUB layout declarations that
 * conflict with the reader's writing mode and pagination.
 *
 * Processing stays within CSS rule boundaries, so a missing final semicolon
 * cannot remove the first declaration of the next rule.
 */
...
private fun String.sanitizeCalibreDeclaration(stripHeight: Boolean): String? =
    when (propertyName()) {
        ...
        "text-indent" -> {
            val value = substringAfter(':', missingDelimiterValue = "").trim()
...
private fun replacementDeclarations(indent: String, property: String, value: String): String =
    when (property) {
        ...
        "word-break" -> declarations(indent, "word-break" to value)
        "hyphens" -> declarations(
            indent,
            "-webkit-hyphens" to value,
            "-hyphens" to value,
        )
        "text-underline-position" -> declarations(indent, "text-underline-position" to value)
